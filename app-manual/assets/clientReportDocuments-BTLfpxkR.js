import{s as h}from"./main-5Fu1vGUV.js";function s(e){return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function u(e){return`
...
<\/script>`;return e.includes("</body>")?e.replace("</body>",`${n}</body>`):`${e}${n}`}function w(e,t,a=!1,n){const r=s(e),o=n?s(n):r,i=s(t);return`<!DOCTYPE html>
<html lang="fr">
  <head>
    <meta charset="utf-8" />
    <title>${i}</title>
...
</html>`}function b(e,t,a,n={}){const{autoPrint:r=!1}=n,o=a??window.open("about:blank","_blank");if(!o)return!1;try{o.focus()}catch{}o.document.open(),o.document.write(g(e,t,r)),o.document.close();try{o.focus()}catch{}return!0}function y(e,t,a,n={}){const{autoPrint:r=!1,sourceUrl:o}=n,i=a??window.open("about:blank","_blank");if(!i)return!1;try{i.focus()}catch{}i.document.open(),i.document.write(w(e,t,r,o)),i.document.close();try{i.focus()}catch{}return!0}function p(e){return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function v(e,t){return`<!DOCTYPE html>
<html lang="fr">
  <head>
    <meta charset="utf-8" />
    <title>${p(e)}</title>
...
</html>`}function m(e,t=!0){if(!t||!e)return;const a=()=>{try{e.focus()}catch{}};[0,120,320,640].forEach(n=>{window.setTimeout(a,n)})}function x(){const e=()=>{try{window.focus()}catch{}};[0,80,180].forEach(t=>{window.setTimeout(e,t)})}function f(e="Ouverture du document",t="Préparation du fichier…",a={}){const{activate:n=!0}=a,r=window.open("about:blank","_blank");if(!r)throw new Error("Le navigateur a bloque l'ouverture du fichier");try{r.document.open(),r.document.write(v(e,t)),r.document.close()}catch(o){console.error("Unable to prepare pending document window",o)}return n?m(r,!0):x(),r}async function l(e,t,a=3600){if(!e||!t)throw new Error("Fichier indisponible");const{data:n,error:r}=await h.storage.from(e).createSignedUrl(t,a);if(r||!(n!=null&&n.signedUrl))throw r??new Error("Impossible d'ouvrir le fichier");return n.signedUrl}async function F(e,t,a=3600,n){if(!e||!t)throw new Error("Fichier indisponible");const r=f();try{const o=await l(e,t,a);return r.location.replace(o),m(r),o}catch(o){try{r.close()}catch(i){console.error("Unable to close pending document window",i)}throw o}}function P(e){return e.replace(/\.[a-z0-9]+$/i,"")}function E(e){return/^rapport-(client|intervention)-[a-z0-9-]+-\d{8}-\d{6}\.(html|pdf)$/i.test(e.trim())}function L(e){var a,n,r;const t=(a=e.file_name)==null?void 0:a.trim();return t&&!E(t)?P(t).replace(/\bBON\b/g,"Bon").replace(/^Rapport client\b/i,"Rapport d'intervention"):(n=e.bon_number)!=null&&n.trim()?`Rapport d'intervention commande ${e.bon_number.trim()}`:(r=e.task_title)!=null&&r.trim()?`Rapport d'intervention - ${e.task_title.trim()}`:"Rapport d'intervention"}function k(e){var r,o,i;const t=((r=e.file_name)==null?void 0:r.toLowerCase())??"",a=((o=e.storage_path)==null?void 0:o.toLowerCase())??"";return(((i=e.mime_type)==null?void 0:i.toLowerCase())??"").includes("text/html")||t.endsWith(".html")||a.endsWith(".html")}async function S(e,t=3600,a){if(!e.storage_bucket||!e.storage_path)throw new Error("Impossible d'ouvrir le rapport d'intervention");const n=L(e),r=a??f(n,"Préparation du rapport d'intervention…");if(k(e))try{const o=await l(e.storage_bucket,e.storage_path,t),i=await fetch(o);if(!i.ok)throw new Error("Impossible d'ouvrir le rapport d'intervention");const c=await i.text();if(!b(c,n,r,{autoPrint:!0}))throw new Error("Le navigateur a bloque l'ouverture du rapport d'intervention");return o}catch(o){try{r.close()}catch(i){console.error("Unable to close pending client report window",i)}throw o}try{const o=await l(e.storage_bucket,e.storage_path,t),i=await fetch(o);if(!i.ok)throw new Error("Impossible d'ouvrir le rapport d'intervention");const c=await i.blob(),d=URL.createObjectURL(c);if(!y(d,n,r,{autoPrint:!0,sourceUrl:d}))throw new Error("Le navigateur a bloque l'ouverture du rapport d'intervention");return d}catch(o){try{r.close()}catch(i){console.error("Unable to close pending client report window",i)}throw o}}export{b as a,S as b,F as c,L as g,f as o};
